...
app.post('/api/personas', async (req, res) => {
    try {
        if (!req.body.nombre || !req.body.apellido || !req.body.mail || !req.body.alias) {
            throw new Error('Todos los datos son obligatorios para registrar a una persona');
        }
        let query = 'SELECT * FROM persona WHERE mail = ? AND alias = ?';
        let respuesta = await qy(query, [req.body.mail, req.body.alias]);

        if (respuesta.length > 0) {
            throw new Error('Ya se encuentra registrado con el mismo email o alias otro usuario, por favor verificar los datos.');
        }
        ...
        res.send({ 'respuesta': respuesta });
        console.log(`Se registro correctamente al cliente ${req.body.nombre} ${req.body.apellido}`);
...
app.put('/api/personas/:id', async (req, res) => {
    ...
    try {
        ...
        console.log(`Se modificaron los datos correctamente: Nombre:${req.body.nombre} Apellido:${req.body.apellido} Alias:${req.body.alias}`);
...
app.post('/api/libros', async (req, res) => {
    try {
        ...
        query = 'INSERT INTO libros (nombre, descripcion, id_genero, id_persona) VALUE (?,?,?,?)';
...
app.get('/api/libros/:id', async (req, res) => {
    try {
        ...
        let query = 'SELECT nombre, descripcion, id_genero, id_persona FROM libros WHERE id = ?';
        let respuesta = await qy(query, [req.params.id]);
        ...
        query = 'SELECT id, nombre, descripcion, id_genero, id_persona FROM libros WHERE id = ?';
        respuesta = await qy(query, [req.params.id]);
        console.log(respuesta);
    }
    catch (e) {
...
app.put('/api/libros/prestar/:id', async (req, res) => {
    ...
    try {
        if (!req.body.id_persona) {
            throw new Error('Es obligatorio indicar el id de la persona para poder prestarle el libro.');
        }

        const libro = 'SELECT * FROM libros WHERE id = ?';
        const respuestaLibro = await qy(libro, [req.params.id]);
        if (respuestaLibro.length < 1) {
            throw new Error('El libro que quieres prestar no esta dentro de nuestra biblioteca');
        }
        ...
        if (respuesta.length > 0) {
            ...
            throw new Error('El libro se encuentra prestado, debe aguardar su devolucion');
        };
    }
    catch (e) {
        console.error(e.message);
